...
class CDPBrowserManager:
    ...
    async def launch_and_connect(
        ...
        """
        try:
            managed = os.environ.get("MEDIAWORKBENCH_SESSION")
            if managed:
                import json
                session = json.loads(managed)
                self._workbench_owned = True
                self.browser = await playwright.chromium.connect_over_cdp(session["endpoint"])
                self.browser_context = self.browser.contexts[0]
                self._initial_pages = list(self.browser_context.pages)
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        if config.SAVE_LOGIN_STATE:
            user_data_dir = os.path.join(
                os.getenv("MEDIAWORKBENCH_BROWSER_ROOT", os.path.join(os.getcwd(), "browser_data")),
    ...
    async def cleanup(self, force: bool = False):
        ...
        """
        if self._workbench_owned:
            if self.browser_context and getattr(config, "WORKBENCH_SESSION_ACTION", "login") != "open":
                for page in self.browser_context.pages:
                    if page not in self._initial_pages:
                        try:
                            await page.close()
                        except Exception:
                            pass
            self.browser_context = self.browser = None
            return
        try:
