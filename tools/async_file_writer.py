...
_search_run_times: dict[tuple[str, str, str], str] = {}
...
class AsyncFileWriter:
    def __init__(self, platform: str, crawler_type: str):
        ...
        first_keyword = config.KEYWORDS.split(",", 1)[0].strip()
        safe_keyword = re.sub(r'[<>:"/\\|?*\x00-\x1f]+', "_", first_keyword).strip(" ._")[:50] or "未命名"
        search_run_key = (platform, config.SAVE_DATA_PATH or "", config.KEYWORDS)
        if search_run_key not in _search_run_times:
            _search_run_times[search_run_key] = utils.get_current_time().replace(" ", "_").replace(":", "-")
        self.search_file_tag = f"{safe_keyword}_{_search_run_times[search_run_key]}"
    ...
    def _get_file_path(self, file_type: str, item_type: str) -> str:
        ...
        suffix = self.search_file_tag if self.crawler_type == "search" else utils.get_current_date()
        file_name = f"{self.crawler_type}_{item_type}_{suffix}.{file_type}"
    ...
    async def write_to_jsonl(self, item: Dict, item_type: str):
        ...
        async with self.lock:
            ...
            if os.getenv("MEDIAWORKBENCH_JOB_ID"):
                from workbench.queue import collect_record
                collect_record(item, item_type, self.platform)
