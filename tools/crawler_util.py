...
LOGIN_QRCODE_PATH = Path(os.getenv("MEDIAWORKBENCH_LOGIN_DIR", str(Path(__file__).resolve().parent.parent / "data"))) / ".login_qrcode.png"
LOGIN_QRCODE_REFRESH_PATH = (
    Path(os.getenv("MEDIAWORKBENCH_LOGIN_DIR", str(Path(__file__).resolve().parent.parent / "data"))) / ".login_qrcode_refresh"
)


def crawl_limit_reached(current: int, limit: int) -> bool:
    return limit > 0 and current >= limit


def register_page_ids(seen: set, page_ids: List[str]) -> bool:
    new_ids = {page_id for page_id in page_ids if page_id} - seen
    seen.update(new_ids)
    return bool(new_ids)


def consume_login_qrcode_refresh() -> bool:
    if not LOGIN_QRCODE_REFRESH_PATH.is_file():
        return False
    LOGIN_QRCODE_REFRESH_PATH.unlink(missing_ok=True)
    return True
...
def show_qrcode(qr_code) -> None:  # type: ignore
    ...
    LOGIN_QRCODE_PATH.parent.mkdir(parents=True, exist_ok=True)
    new_image.save(LOGIN_QRCODE_PATH, format="PNG")
    from workbench.auth import report
    report("waiting_scan")
