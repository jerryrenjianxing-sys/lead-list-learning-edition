...
async def check_environment():
    ...
    try:
        # Reuse the Python interpreter that is already running the API. This keeps
        # the packaged app independent from a globally installed uv executable.
...
if os.path.exists(WEBUI_DIR):
    ...
    platforms_dir = os.path.join(WEBUI_DIR, "platforms")
    if os.path.exists(platforms_dir):
        app.mount("/platforms", StaticFiles(directory=platforms_dir), name="platforms")
    # Mount other static files (e.g., vite.svg)
    app.mount("/static", StaticFiles(directory=WEBUI_DIR), name="webui-static")


def run_api() -> None:
    """Start the local control API with an explicit opt-in for network access."""
