...
class ZhihuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if not await managed_login("zhihu", self, self.zhihu_client, ZhiHuLogin) and not await self.zhihu_client.pong():
    ...
    async def search(self) -> None:
        ...
        if 0 < crawl_limit < zhihu_limit_count:
            crawl_limit = zhihu_limit_count
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while not utils.crawl_limit_reached(
                (page - start_page + 1) * zhihu_limit_count - 1,
                crawl_limit,
            ):
                if page < start_page:
                ...
                try:
                    ...
                    if not utils.register_page_ids(
                        seen_page_ids, page_content_ids
                    ):
                        utils.logger.warning(
                            f"[ZhihuCrawler.search] Repeated page for keyword:{keyword}, page:{page}; stopping pagination"
                        )
                        break
