...
class ZhiHuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_all_anwser_by_creator(self, url_token: str, crawl_interval: float = 1.0, callback: Optional[Callable] = None) -> List[ZhihuContent]:
        ...
        while not is_end:
            ...
            if not contents:
                break
            page_content_ids = [content.content_id for content in contents]
            if not utils.register_page_ids(seen_page_ids, page_content_ids):
                utils.logger.warning(
                    f"[ZhiHuClient.get_all_anwser_by_creator] "
                    f"Creator {url_token} returned no new content, stop pagination"
                )
                break
            if callback:
    ...
    async def get_all_articles_by_creator(
        ...
        while not is_end:
            ...
            if not utils.register_page_ids(seen_page_ids, page_content_ids):
                utils.logger.warning(
                    f"[ZhiHuClient.get_all_articles_by_creator] "
                    f"Creator {url_token} returned no new content, stop pagination"
                )
                break
            if callback:
    ...
    async def get_all_videos_by_creator(
        ...
        while not is_end:
            ...
            if not utils.register_page_ids(seen_page_ids, page_content_ids):
                utils.logger.warning(
                    f"[ZhiHuClient.get_all_videos_by_creator] "
                    f"Creator {url_token} returned no new content, stop pagination"
                )
                break
            if callback:
