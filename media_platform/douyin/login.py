...
class DouYinLogin(AbstractLogin):
    ...
    async def popup_login_dialog(self):
        """Recognize both current and legacy dialogs before clicking behind an overlay."""
        dialog = self.context_page.locator(
            "#login-panel-new:visible, [id^='login-full-panel-']:visible, "
            "#animate_qrcode_container:visible"
        ).first
        try:
            await dialog.wait_for(state="visible", timeout=10000)
            return
        except PlaywrightTimeoutError:
            utils.logger.info("[DouYinLogin.popup_login_dialog] Opening login dialog")
        try:
            await self.context_page.get_by_text("登录", exact=True).filter(visible=True).first.click(timeout=10000)
        except PlaywrightTimeoutError:
            # The automatic modal may appear between the wait and click.
