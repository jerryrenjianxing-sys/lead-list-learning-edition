...
class DouYinClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs):
        ...
        attempts = 1 if getattr(self, "_workbench_auth_probe", False) else 3
            ...
            try:
                ...
                if attempt == attempts - 1:
                    raise DataFetchError(
                        f"Douyin network connection failed after {attempts} attempts",
                        request=exc.request,
                    ) from exc
    ...
    async def get_aweme_all_comments(
        ...
        while comments_has_more and not utils.crawl_limit_reached(len(result), max_count):
            ...
            if not comments:
                break
            page_comment_ids = [str(comment.get("cid") or "") for comment in comments]
            new_comments = [
                comment
            ...
            for comment in comments:
                ...
                if reply_comment_total > 0:
                    ...
                    while sub_comments_has_more:
                        request_sub_cursor = sub_comments_cursor
                        ...
                        sub_cursor_stalled = (
                            ...
                            and sub_comments_cursor == request_sub_cursor
                        )
    ...
    async def get_all_user_aweme_posts(self, sec_user_id: str, callback: Optional[Callable] = None):
        ...
        while posts_has_more == 1:
            ...
            if not aweme_list:
                break
            page_aweme_ids = [
                str(aweme.get("aweme_id") or "") for aweme in aweme_list
            ]
            if not utils.register_page_ids(seen_page_ids, page_aweme_ids):
                utils.logger.warning(
                    f"[DouYinClient.get_all_user_aweme_posts] Repeated page for sec_user_id:{sec_user_id}; stopping pagination"
                )
                break
            if callback:
