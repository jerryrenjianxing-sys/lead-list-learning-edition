...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if not await managed_login("bili", self, self.bili_client, BilibiliLogin) and not await self.bili_client.pong():
    ...
    async def search_by_keywords(self):
        ...
        if 0 < crawl_limit < bili_limit_count:
            crawl_limit = bili_limit_count
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while not utils.crawl_limit_reached(
                (page - start_page + 1) * bili_limit_count - 1,
                crawl_limit,
            ):
                if page < start_page:
                ...
                if not video_list:
                    ...
                    break
                page_video_ids = [
                    str(video_item.get("aid") or video_item.get("bvid") or "")
                    for video_item in video_list
                ]
                if not utils.register_page_ids(seen_page_ids, page_video_ids):
                    utils.logger.warning(
                        f"[BilibiliCrawler.search_by_keywords] Repeated page for keyword:{keyword}, page:{page}; stopping pagination"
                    )
                    break
                ...
                task_list = []
                try:
                    task_list = [self.get_video_info_task(aid=video_item.get("aid"), bvid=video_item.get("bvid") or "", semaphore=semaphore) for video_item in video_list]
    ...
    async def search_by_keywords_in_time_range(self, daily_limit: bool):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            for day in pd.date_range(start=config.START_DAY, end=config.END_DAY, freq="D"):
                if daily_limit and utils.crawl_limit_reached(
                ...
                if not daily_limit and utils.crawl_limit_reached(
                    total_notes_crawled_for_keyword,
                    config.CRAWLER_MAX_NOTES_COUNT,
                ):
                    utils.logger.info(f"[BilibiliCrawler.search] Reached CRAWLER_MAX_NOTES_COUNT limit for keyword '{keyword}', skipping remaining days.")
                ...
                while True:
                    ...
                    try:
                        ...
                        if not utils.register_page_ids(seen_page_ids, page_video_ids):
                            utils.logger.warning(
                                f"[BilibiliCrawler.search] Repeated page for keyword:{keyword}, date:{day.ctime()}, page:{page}; stopping pagination"
                            )
                            break
    ...
    async def get_creator_videos(self, creator_id: int):
        ...
        seen_page_ids = set()
        crawled_count = 0
        creator_limit = config.CRAWLER_MAX_NOTES_COUNT
        while not utils.crawl_limit_reached(
            crawled_count, creator_limit
        ):
            result = await self.bili_client.get_creator_videos(creator_id, pn, ps)
            video_list = result.get("list", {}).get("vlist") or []
            if not video_list:
                break
            page_video_ids = [
                str(video.get("bvid") or video.get("aid") or "")
                for video in video_list
            ]
            if not utils.register_page_ids(seen_page_ids, page_video_ids):
                utils.logger.warning(
                    f"[BilibiliCrawler.get_creator_videos] Repeated page for creator:{creator_id}, page:{pn}; stopping pagination"
                )
                break
            videos_to_process = video_list
            if creator_limit:
                remaining = creator_limit - crawled_count
                videos_to_process = videos_to_process[:remaining]
            video_bvids_list = [
                str(video.get("bvid"))
            ...
            crawled_count += len(videos_to_process)
