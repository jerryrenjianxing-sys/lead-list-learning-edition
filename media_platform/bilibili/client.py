...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_video_all_comments(
        ...
        """
        Get first-level video comments.

        Second-level replies are emitted through ``callback`` when
        ``is_fetch_sub_comments`` is enabled; they are not included in the
        returned list.
        :param video_id:
        ...
        :return: First-level comment list
        """
        result = []
        ...
        max_retries = 3
        is_first_page = True
        while not is_end and not utils.crawl_limit_reached(len(result), max_count):
            request_page = next_page
            request_is_first_page = is_first_page
            comments_res = None
            ...
            if not comment_list:
                break
            page_comment_ids = [str(comment.get("rpid") or "") for comment in comment_list]
            new_comments = [
                comment
                for comment, comment_id in zip(comment_list, page_comment_ids)
            ...
            page_stalled = not is_end and next_page == request_page and not request_is_first_page
            if max_count > 0 and len(result) + len(comment_list) > max_count:
