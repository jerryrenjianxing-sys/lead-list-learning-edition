...
class TieBaCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if not await managed_login("tieba", self, self.tieba_client, BaiduTieBaLogin) and not await self.tieba_client.pong(browser_context=self.browser_context):
    ...
    async def search(self) -> None:
        ...
        if 0 < crawl_limit < tieba_limit_count:
            crawl_limit = tieba_limit_count
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while not utils.crawl_limit_reached(
                (page - start_page + 1) * tieba_limit_count - 1,
                crawl_limit,
            ):
                if page < start_page:
                ...
                try:
                    ...
                    page_note_ids = [
                        str(
                            getattr(note_detail, "note_id", "")
                            or getattr(note_detail, "tid", "")
                        )
                        for note_detail in notes_list
                    ]
                    if not utils.register_page_ids(seen_page_ids, page_note_ids):
                        utils.logger.warning(
                            f"[BaiduTieBaCrawler.search] Repeated page for keyword:{keyword}, page:{page}; stopping pagination"
                        )
                        break
                    utils.logger.info(
        ...
        for tieba_name in config.TIEBA_NAME_LIST:
            ...
            while not utils.crawl_limit_reached(
                ...
                if not note_list:
                    ...
                    break
                page_note_ids = [
                    str(getattr(note, "note_id", "") or getattr(note, "tid", ""))
                    for note in note_list
                ]
                if not utils.register_page_ids(seen_page_ids, page_note_ids):
                    utils.logger.warning(
                        f"[BaiduTieBaCrawler.get_specified_tieba_notes] Repeated page for tieba:{tieba_name}, page:{page_number}; stopping pagination"
                    )
                    break
