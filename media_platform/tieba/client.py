...
class BaiduTieBaClient(AbstractApiClient):
    ...
    async def get_note_all_comments(
        ...
        while (
            note_detail.total_replay_page >= current_page
            and not utils.crawl_limit_reached(len(result), max_count)
        ):
            utils.logger.info(
    ...
    async def get_all_notes_by_creator_url(
        ...
        while max_note_count == 0 or len(result) < max_note_count:
            ...
            if not utils.register_page_ids(
                seen_page_ids, [str(thread_id) for thread_id in thread_id_list]
            ):
                utils.logger.warning(
                    f"[BaiduTieBaClient.get_all_notes_by_creator_url] "
                    f"Repeated page for portrait:{portrait}; stopping pagination"
                )
                break
