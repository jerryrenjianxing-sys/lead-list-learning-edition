...
class WeiboClient(ProxyRefreshMixin):
    ...
    async def get_note_all_comments(
        ...
        while not is_end and not utils.crawl_limit_reached(len(result), max_count):
            request_cursor = (max_id, max_id_type)
            ...
            if not comment_list:
                break
            page_comment_ids = [
                str(comment.get("id") or comment.get("idstr") or "")
            ...
            cursor_stalled = not is_end and (max_id, max_id_type) == request_cursor
    ...
    async def get_all_notes_by_creator_id(
        ...
        while notes_has_more:
            ...
            page_note_ids = [
                str(
                    (note.get("mblog") or {}).get("id")
                    or (note.get("mblog") or {}).get("idstr")
                    or ""
                )
                for note in notes
            ]
            if not utils.register_page_ids(seen_page_ids, page_note_ids):
                utils.logger.warning(
                    f"[WeiboClient.get_all_notes_by_creator] Repeated page for creator_id:{creator_id}; stopping pagination"
                )
                break
            if callback:
