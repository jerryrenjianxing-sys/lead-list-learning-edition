...
class WeiboCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if not await managed_login("wb", self, self.wb_client, WeiboLogin) and not await self.wb_client.pong():
    ...
    async def search(self):
        ...
        if 0 < crawl_limit < weibo_limit_count:
            crawl_limit = weibo_limit_count
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while not utils.crawl_limit_reached(
                (page - start_page + 1) * weibo_limit_count - 1,
                crawl_limit,
            ):
                if page < start_page:
                ...
                if not note_list:
                    utils.logger.info(
                        f"[WeiboCrawler.search] No more notes for keyword:{keyword}"
                    )
                    break
                page_note_ids = [
                    str(
                        (note_item.get("mblog") or {}).get("id")
                        or (note_item.get("mblog") or {}).get("idstr")
                        or ""
                    )
                ...
                if not utils.register_page_ids(seen_page_ids, page_note_ids):
                    utils.logger.warning(
                        f"[WeiboCrawler.search] Repeated page for keyword:{keyword}, page:{page}; stopping pagination"
                    )
                    break
                ...
                for note_item in note_list:
                    if note_item:
                        ...
                        if mblog:
                            note_id_list.append(
                                mblog.get("id") or mblog.get("idstr")
                            )
