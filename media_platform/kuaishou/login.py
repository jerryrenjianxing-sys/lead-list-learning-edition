...
class KuaishouLogin(AbstractLogin):
    ...
    async def check_login_state(self) -> bool:
        ...
        """
        if utils.consume_login_qrcode_refresh():
            await self.context_page.reload()
            await self._publish_qrcode()
    ...
    async def login_by_qrcode(self):
        ...
        utils.LOGIN_QRCODE_REFRESH_PATH.unlink(missing_ok=True)

        if not await self._publish_qrcode():
            utils.logger.info("[KuaishouLogin.login_by_qrcode] login failed , have not found qrcode please check ....")
            sys.exit(1)

        utils.logger.info("[KuaishouLogin.login_by_qrcode] waiting for scan code login, remaining time is 120s")
        try:
    ...
    async def _publish_qrcode(self) -> bool:
