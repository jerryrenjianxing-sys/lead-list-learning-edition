...
class KuaishouCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if not await managed_login("ks", self, self.ks_client, KuaishouLogin) and not await self.ks_client.pong():
    ...
    async def search(self):
        ...
        if 0 < crawl_limit < ks_limit_count:
            crawl_limit = ks_limit_count
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while not utils.crawl_limit_reached(
                (page - start_page + 1) * ks_limit_count - 1,
                crawl_limit,
            ):
                if page < start_page:
                ...
                vision_search_photo: Dict = videos_res.get("visionSearchPhoto") or videos_res
                ...
                video_list = vision_search_photo.get("feeds") or []
                if not video_list:
                    utils.logger.info(
                    ...
                    break
                page_video_ids = [
                    str(video_detail.get("photo", {}).get("id") or "")
                    for video_detail in video_list
                ]
                if not utils.register_page_ids(seen_page_ids, page_video_ids):
                    utils.logger.warning(
                        f"[KuaishouCrawler.search] Repeated page for keyword:{keyword}, page:{page}; stopping pagination"
                    )
                    break
                for video_detail in video_list:
                    ...
                    if self.lead_sink:
                        try:
                            ...
                            utils.logger.error(
                                f"[KuaishouCrawler.search] Failed to record lead content: {ex}"
                            )
                ...
                if vision_search_photo.get("pcursor") == "no_more":
                    break
        ...
        for video_detail in video_details:
            if video_detail is not None:
                ...
                if self.lead_sink:
                    ...
                    video_id = str(photo.get("id") or video_detail.get("id") or "")
                    try:
                        self.lead_sink.record_content(
                            video_id,
                            video_detail,
                        ...
                        utils.logger.error(
                            f"[KuaishouCrawler.get_specified_videos] Failed to record lead content: {ex}"
                        )
    ...
    async def get_video_info_task(
        ...
        async with semaphore:
            try:
                ...
                utils.logger.info(
                    f"[KuaishouCrawler.get_video_info_task] Got video detail: {video_id}"
                )
                return detail
    ...
    async def get_comments(self, video_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                ...
                if self.lead_sink:
                    async def callback(current_video_id: str, comments: List[Dict]):
                        await kuaishou_store.batch_update_ks_video_comments(current_video_id, comments)
                        try:
                            self.lead_sink.record_ks_comments(current_video_id, comments)
                        except Exception as ex:
                            utils.logger.error(
                                f"[KuaishouCrawler.get_comments] Failed to record lead comments: {ex}"
                            )
    ...
    async def _collect_lead_profile_refs(self, refs: List[Dict]) -> bool:
        ...
        for ref in refs:
            if ref.get("profiled"):
                continue
            try:
                profile = await self.ks_client.get_creator_info_by_browser(ref["user_id"])
                if not profile:
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            user_data_dir = os.path.join(
                os.getenv("MEDIAWORKBENCH_BROWSER_ROOT", os.path.join(os.getcwd(), "browser_data")), config.USER_DATA_DIR % config.PLATFORM
            )  # type: ignore
            ...
            if video_detail is not None:
                ...
                if self.lead_sink:
                    ...
                    try:
                        ...
                        utils.logger.error(
                            f"[KuaishouCrawler.fetch_creator_video_detail] Failed to record lead content: {ex}"
                        )
