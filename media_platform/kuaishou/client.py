...
class KuaiShouClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_video_all_comments(
        ...
        while pcursor != "no_more" and not utils.crawl_limit_reached(len(result), max_count):
            request_cursor = pcursor
            ...
            if not comments:
                break
            page_comment_ids = [
                str(comment.get("comment_id") or comment.get("commentId") or "")
            ...
            cursor_stalled = pcursor != "no_more" and pcursor == request_cursor
            if max_count > 0 and len(result) + len(comments) > max_count:
                comments = comments[: max_count - len(result)]
            if callback and comments:  # If there is a callback function, execute the callback function
    ...
    async def get_comments_all_sub_comments(
        ...
        for comment in comments:
            ...
            while sub_comment_pcursor != "no_more":
                request_sub_cursor = sub_comment_pcursor
                ...
                page_sub_comment_ids = [
                    str(
                        sub_comment.get("comment_id")
                        or sub_comment.get("commentId")
                        or ""
                    )
                ...
                new_sub_comments = [
                    ...
                    for sub_comment, sub_comment_id in zip(
                        sub_comments, page_sub_comment_ids
                    )
                ...
                sub_cursor_stalled = (
                    ...
                    and sub_comment_pcursor == request_sub_cursor
                )
    ...
    async def get_creator_info(self, user_id: str) -> Dict:
        ...
        return (visionProfile.get("visionProfile") or {}).get("userProfile")

    async def get_creator_info_by_browser(self, user_id: str) -> Dict:
        """Read the complete public profile, including userDefineId, from the profile page."""
        await self.playwright_page.goto(
            f"{self._rest_host}/profile/{quote(user_id, safe='')}",
            wait_until="domcontentloaded",
        )
        profile_handle = await self.playwright_page.wait_for_function(
            """(userId) => {
                const seen = new Set();
                const find = (value, depth) => {
                    if (!value || typeof value !== "object" || seen.has(value) || depth > 10) return null;
                    seen.add(value);
                    if (value.profile?.user_id === userId) return value;
                ...
                return find(window.INIT_STATE, 0) || false;
            }""",
            arg=user_id,
            timeout=15_000,
        )
        profile = await profile_handle.json_value()
    ...
    async def get_all_videos_by_creator(
        ...
        while pcursor != "no_more":
            ...
            if not videos:
                break
            page_video_ids = [
                str(video.get("photo", {}).get("id") or "") for video in videos
            ]
            if not utils.register_page_ids(seen_page_ids, page_video_ids):
                utils.logger.warning(
                    f"[KuaiShouClient.get_all_videos_by_creator] Repeated page for user_id:{user_id}; stopping pagination"
                )
                break
