...
async def test_merge_kills_subprocess_when_cancelled(tmp_path, monkeypatch):
    ...
    async def sleeping_process(*args, **kwargs):
        ...
        return process
    monkeypatch.setattr(ffmpeg_module, "ffmpeg_path", lambda: sys.executable)
    monkeypatch.setattr(ffmpeg_module.asyncio, "create_subprocess_exec", sleeping_process)
    task = asyncio.create_task(ffmpeg_module.merge_audio_video(tmp_path / "v.m4s", tmp_path / "a.m4s", tmp_path / "o.mp4", timeout=60))
    for _ in range(50):
        if processes: break
        await asyncio.sleep(0.02)
    assert processes, "subprocess must be running before cancellation"
    task.cancel()
    with pytest.raises(asyncio.CancelledError):
        await task
    assert processes[0].returncode is not None, "cancelled merge left a running child"
