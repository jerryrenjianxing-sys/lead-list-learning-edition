    ...
    def main(
        ...
        max_comments_count_singlenotes: Annotated[
            ...
            typer.Option(
                ...
                help="Maximum first-level comments per post/video (0 = unlimited)",
                rich_help_panel="Comment Configuration",
            ),
        ...
        crawler_max_notes_count: Annotated[
            ...
            typer.Option(
                ...
                help="Maximum videos/posts to crawl (0 = unlimited)",
                rich_help_panel="Basic Configuration",
            ),
        ...
        required_input = {
            CrawlerTypeEnum.SEARCH: ("--keywords", keywords),
            CrawlerTypeEnum.DETAIL: ("--specified_id", specified_id),
            CrawlerTypeEnum.CREATOR: ("--creator_id", creator_id),
        }.get(crawler_type)
        if required_input and not required_input[1].strip(" \t\r\n,，"):
            raise typer.BadParameter(
                f"{required_input[0]} is required for {crawler_type.value} mode",
                param_hint=required_input[0],
            )
